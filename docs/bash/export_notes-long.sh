...
mkdir -p pdf
# pandoc --from=markdown+rebase_relative_paths -s -o pdf/notes-long.pdf --metadata-file=metadata.yaml --pdf-engine=pdflatex $(cat "$temp_file")
pandoc --from=markdown+rebase_relative_paths -s -o pdf/notes-long.pdf --metadata-file=metadata.yaml --pdf-engine=xelatex $(cat "$temp_file")
